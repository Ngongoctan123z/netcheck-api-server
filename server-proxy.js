...
app.post("/api/check-proxy-strong", async (req, res) => {
  try {
    ...
    if (!proxy || typeof proxy !== "string") return res.status(400).json({ error: "Missing or invalid proxy" });

    const allowedTypes = ["http", "https", "socks4", "socks5"];
    if (!type || !allowedTypes.includes(type)) return res.status(400).json({
      error: "Invalid proxy type. Supported: http, https, socks4, socks5"
    });

    timeoutMs = parseInt(timeoutMs, 10);
    if (Number.isNaN(timeoutMs) || timeoutMs < 1000 || timeoutMs > MAX_TIMEOUT_MS) timeoutMs = DEFAULT_TIMEOUT_MS;

    const parsed = parseProxyString(proxy);
    if (!parsed) return res.status(400).json({ error: "Required format: user:pass@ip:port" });
